...
pub fn peaks(path: &Path, requested_bins: usize) -> Result<Vec<f32>> {
    ...
    let channels = track.codec_params.channels.map(|value| value.count()).unwrap_or(1);
    ...
    while let Ok(packet) = format.next_packet() {
        ...
        for sample in buffer.samples() {
            let bin = if total_samples > 0 {
                sample_index.saturating_mul(bin_count) / total_samples
            } else {
                sample_index % bin_count
            };
            if bin < bin_count {
