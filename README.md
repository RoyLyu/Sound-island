...
> 当前版本：`0.7.0` UCS 工作流版。素材库默认为空，不包含任何示例或 Mock 数据。
...
- 中英文常见关键词扩展搜索
- 使用官方 Universal Category System 8.2.1 的中文、英文、同义词与 CatID 对文件名和目录语义分类
- 82 个 UCS 母类采用可滚动层级列表，展开后显示当前素材库实际存在的 UCS 子类
- 本地试听、进度定位、收藏；列表音频可作为真实文件拖到桌面、Finder 或外部制作软件
- 右键声音可复制真实文件对象、送入声音实验室、定位原文件或切换收藏；`Cmd/Ctrl + C` 可直接复制当前选择
- 真实波形总览：Rust 在本机解码音频并向界面返回归一化峰值
- 键盘工作流：`Space` 播放，`W` / `S` 选择，`F` 收藏，`Z` 撤回，`Tab` 搜索，`Cmd/Ctrl + E` 导出副本
- 智能视图：我的收藏与最近播放
- 主分类可展开/取消选择；母文件夹显示全部层级的后代目录及递归音频数量
- 本地素材库提供明确的上移/下移按钮，排列顺序持久保存到本机 SQLite
- 声音实验室：实时三段 EQ、混响、延迟、失真、九种高辨识度预设、A/B 对比与自定义调整
...
- 底部试听区显示可跳转的预计算波形，走带和读秒使用独立动画帧平滑更新
- 底部试听区精简低频操作，放大音量控制，并以专用徽章区分 Mono、Stereo、Quad、5.1、7.1 与其他多声道
- 声音实验室的拓宽、立体化、空间迁移与遮挡采用与一键方案一致的左侧卡片入口
- 声音实验室显示一次性计算的轻量波形并保留独立拖动进度；参数重建防抖，播放时不执行实时频谱分析
...
导入时，应用会用 UCS 8.2.1 的 Category、SubCategory、CatID 及中英文同义词分析文件名和完整目录路径，例如：

| 文件名 | UCS 母类 | UCS 子类 |
|---|---|---|
| `008 Jet Takes Off.wav` | 航空器 / AIRCRAFT | 喷气式飞机 / JET · AEROJet |
| `Leather_Boots_Footsteps.aif` | 脚步 / FOOTSTEPS | 人类 / HUMAN · FEETHmn |
| `Metal Door Slam.wav` | 门 / DOORS | 金属门 / METAL · DOORMetl |
| `000000.wav` | 待归类 / UNCATEGORIZED | 未匹配 / MISC |

分类仅存在于声屿数据库中，不会在硬盘上移动文件。只能可靠判断母类时进入「未细分 / UNSPECIFIED」，母类也无法判断时进入「待归类 / UNCATEGORIZED」，避免擅自猜测。升级首次启动会在本地重新分类已有索引，不读取或上传音频内容。

匹配器位于 [`src-tauri/src/classify.rs`](src-tauri/src/classify.rs)，UCS 规则快照位于 [`src-tauri/src/ucs_catalog.rs`](src-tauri/src/ucs_catalog.rs)。仓库不包含下载来源表格。
