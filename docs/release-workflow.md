...
- 搜索、选择与 Space 试听；底部预计算波形、游标和读秒应连续流畅；
- 输入搜索词后结果来自全部素材库，不受当前分类或文件夹筛选影响；
- UCS 8.2.1 母类可独立滚动，展开后只显示当前库存在的子类；二次点击收回并取消选择；
- 母文件夹能展示任意深度的后代目录；
- 使用上移/下移按钮调整素材库顺序后重启应用，顺序保持不变；
- 右侧详情突出显示 UCS CatID、格式及 Mono/Stereo/5.1 等声道布局；
- `F` 收藏、`Z` 撤回、`Tab` 搜索、`Cmd/Ctrl + C` 复制真实文件对象、`Cmd/Ctrl + E` 导出；
- 右键声音可复制、送入声音实验室、定位与切换收藏；
- 中文显示名只读取原始音频名，真实路径和硬盘文件名不变；
- 拖出音频使用 copy 模式；
- 声音实验室左侧用统一卡片进入拓宽、单声道立体化、空间/遮挡；验证 420 峰值静态波形、短进度条、播放游标、A/B、九种方案、相位相关度和 24-bit WAV 导出；
